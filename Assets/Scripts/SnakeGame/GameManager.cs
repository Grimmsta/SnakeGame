...
namespace Grimm //In case someone else is writing scripts with i.e the same methods (not likley here but...)
{
    public class GameManager : MonoBehaviour
    {
        ...
        private void Update() //Cheks for inputs or the state of the game 
        {
            ...
            else if (timer > moveRate && isAStar)
            {
        ...
        private void MoveAStar()
        {
            ...
            Node targetNode = pathFinding.path[0]; //Get the nodes around you
            ...
            else //If there is a target node around the player, the position of that node becomes the new position of the player
            {
                ...
                else
                {
                    ...
                    Node previousNode = targetNode.parent;

                    //availableNodes.Add(currentNode);

                    if (isScore)
                    {
                        tail.Add(CreateTailNode(currentNode.x + x, currentNode.y + y)); //Adds a node to your tail
                        //availableNodes.Remove(currentNode); //Removes the amount of available nodes since your tail is growing 
                        grid[currentNode.x, currentNode.y].walkable = false;
        ...
        void MovePlayer() //Moves the player
        {
            ...
            else //If there is a target node around the player, the position of that node becomes the new position of the player
            {
                ...
                else
                {
                    ...
                    if (isScore)
                    {
                        tail.Add(CreateTailNode(currentNode.x, currentNode.y)); //Adds a node to your tail
                                                                                // availableNodes.Remove(currentNode); //Removes the amount of available nodes since your tail is growing 
                    }
